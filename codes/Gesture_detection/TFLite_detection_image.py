...
树莓派AI实验
目标检测
基于ssd_mobilenet_v2量化模型的Tensorflow Lite模型实现单个图像‘0’，‘1’手势识别
'''
import argparse
import cv2
import numpy as np
...
# 图片路径
parser.add_argument('--image', default='Test_images\hand.JPG')
...
if __name__ == '__main__':
    ...
    if ((scores[i] > Net.min_conf_threshold) and (scores[i] <= 1.0)):  # 只要结果大于最低置信度
        ...
        ymax = int(min(src.shape[0], (boxes[i][2] * Net.imH) + ROI.dy))
        xmax = int(min(src.shape[1], (boxes[i][3] * Net.imW) + ROI.dx))

...
树莓派AI实验
目标检测
基于ssd_mobilenet_v2量化模型的Tensorflow Lite模型实现网络摄像头‘0’，‘1’手势识别
'''
import argparse
import cv2
import numpy as np
...
if __name__ == '__main__':
    ...
    vw, vh = args.resolution.split('x')
    vw, vh = int(vw), int(vh)

    videostream = cv2.VideoCapture(0)
    _ = videostream.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG'))
    _ = videostream.set(3, vw)
    _ = videostream.set(4, vh)
    ...
    while (True):
        ...
        _, src = videostream.read()  # 从视频流中读入图片
        ...
        # 可视化神经网络推理速率
        cv2.putText(src, 'FPS: {0:.2f}'.format(frame_rate_calc), (30, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0),
                    2,
                    cv2.LINE_AA)

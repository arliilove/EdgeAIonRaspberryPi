...
树莓派AI实验
目标检测
基于ssd_mobilenet_v2量化模型的Tensorflow Lite模型实现网络摄像头数字九宫格识别
'''
import argparse
import cv2
import numpy as np
...
parser.add_argument('--modeldir', default='../TFLite_model/Model_Sudoku_6')
# .tflite文件名
...
''' 设定相关参数 '''
is_showed = {'show_mask': False, 'show_paper': False, 'show_paper_preprocessed': False, 'show_square_obtained': False,
             'show_sudoku': True}
...
if __name__ == '__main__':
    ...
    while True:
        ...
        """ 可视化结果 """
        try:
            visualize(src, results, x_step=int((1 / 3) * ROI.sudoku.shape[1]),
                      y_step=int((1 / 3) * ROI.sudoku.shape[0]),
